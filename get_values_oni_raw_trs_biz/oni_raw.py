...
async def main():
  asyncio.create_task(environment('prod'))
...
if __name__ == "__main__":
  ...
  path = "{adl_path}{default_dir}".format(adl_path=var, default_dir=loop.run_until_complete(main())) 
...
async def environment(value) -> 'value_dev_or_prod':
  print("running coroutine...")
  env = value
  var_adls_uri = 'abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net'
  default_dir = '/'.join(['','tmp','dev'])
...
loop = asyncio.get_event_loop()
var = 'abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net'
path = "{adl_path}{default_dir}".format(adl_path=var, default_dir=loop.run_until_complete(environment('prod'))) 
