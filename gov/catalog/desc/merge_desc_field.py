...
if env == 'dev': default_dir = '/'.join(['','tmp','dev'])
elif env == 'prod': default_dir = ""
path_lnd_gov = "{adl_path}{default_dir}".format(adl_path=var_adls_uri,default_dir=default_dir) + '/'.join(['','gov','usr_upload','field'])
path_field = "{adl_path}{default_dir}".format(adl_path=var_adls_uri,default_dir=default_dir) + '/'.join(['','gov','tables','field'])

# COMMAND ----------

env, path_lnd_gov, path_field
...
if file_exists == 1:
  ...
  for coluna in colunas_csv:
    if coluna not in ['source_name','source_type','schema_name','table_name','field_name','path']:
