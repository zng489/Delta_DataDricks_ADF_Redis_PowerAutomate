...
path_lnd_gov = "{adl_path}{default_dir}".format(adl_path=var_adls_uri,default_dir=default_dir) + '/'.join(['','gov','usr_upload','source'])
path_source = "{adl_path}{default_dir}".format(adl_path=var_adls_uri,default_dir=default_dir) + '/'.join(['','gov','tables','source'])

# COMMAND ----------

env, path_lnd_gov, path_source
...
if file_exists == 1:
  ...
  update_set = {  ## campos que são atualizados manualmente - por meio de planilha
