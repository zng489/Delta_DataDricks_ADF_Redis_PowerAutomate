...
var_file = {
   'namespace': 'oni',
   'file_folder': 'oms/cid_10/categorias',
   'extension': 'csv',
   'column_delimiter': ';',
   'encoding': 'UTF-8',
   'null_value': ''
...
var_source = "{raw}/usr/{namespace}/{file_folder}".format(raw=raw, namespace=var_file['namespace'], file_folder=var_file['file_folder'])
print(var_source)


var_sink = "{adl_path}{trs}/{namespace}/{file_folder}".format(adl_path=var_adls_uri, trs=trs, namespace=var_file['namespace'], file_folder=var_file['file_folder'])
print(var_sink)
