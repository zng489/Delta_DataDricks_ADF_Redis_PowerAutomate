...
from pyspark.sql.functions import udf, from_utc_timestamp, current_timestamp, lit, input_file_name, monotonically_increasing_id, substring_index, lpad
...
for df_column in df.columns:
  ...
  match = re.search(pattern, df_column)

  if match:
    #print("Pattern found!")
    print(df_column)
    #print("Column name: {df_column}")

    # Verificando se todos os valores de NR_CPF têm exatamente 11 dígitos
    invalid_entries = df.filter(~f.col(df_column).rlike("^\d{11}$"))
    # Verificando se existem entradas inválidas
    if invalid_entries.count() == 0:
      print("Todos os valores têm exatamente 11 dígitos.")
    else:
      print(f"Existem {invalid_entries.count()} entradas inválidas.")
      df = df.withColumn(df_column, f.lpad(f.col(df_column), 11, "0"))
  else: pass
...
for df_column in df.columns:
  ...
  if match:
    ...
    # Verificando se todos os valores de NR_CPF têm exatamente 14 dígitos
    invalid_entries = df.filter(~f.col(df_column).rlike("^\d{14}$"))
    # Verificando se existem entradas inválidas
    if invalid_entries.count() == 0:
      print("Todos os valores têm exatamente 14 dígitos.")
    else:
      print(f"Existem {invalid_entries.count()} entradas inválidas.")
      df = df.withColumn(df_column, f.lpad(f.col(df_column), 14, "0"))
  else: pass
    #print("Pattern not found.")

df = df.withColumn("NR_CNPJ_RAIZ", f.substring("NR_CNPJ_CEI", 1, 8))
