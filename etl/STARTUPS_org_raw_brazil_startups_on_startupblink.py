...
# Assuming these are the original methods:
# cf.list_subdirectory(dbutils, path) -> synchronous
# spark.read -> synchronous (Spark itself is not async-compatible, so we'll use threading for parallelism)

async def explore_and_load_file(file_path, dbutils, var_adls_uri):
    # Simulate loading the CSV with Spark (keeping Spark's sync nature)
    df = (spark.read
                .option("delimiter", ";")
                .option("header", "true")
...
async def explore_subdirectories(path, dbutils, var_adls_uri):
    # dataframes = []
    # Exploring all subdirectories and files asynchronously
    # tasks = []
    # List subdirectories at the top level
        ...
        for file_path in file_paths:
            ...
            for arquivo in arquivos_txt:
                if arquivo.endswith('.xlsx'):
                    ...
                    # tasks.append(explore_and_load_file(file_path, dbutils, var_adls_uri, 'Lista de Startups'))
                else:
                    print(f'There is no file txt in {path}')
    # Wait for all tasks to finish and collect the dataframes
    #dataframes = await asyncio.gather(*tasks)
    return

# COMMAND ----------

async def excelToParquet(adl_uld, file_name_xlsx, sheet_name):
  path_dir, file_name = os.path.split(adl_uld + file_name_xlsx)
  path_dbfs = '/tmp/' + sheet_name
  dbutils.fs.cp(adl_uld + file_name_xlsx, path_dbfs, recurse=True) 
  pd_df = pd.read_excel('/dbfs' + path_dbfs, engine='openpyxl', sheet_name=sheet_name).astype(str)
  sparkDF = spark.createDataFrame(pd_df)
  for column in sparkDF.columns:
    sparkDF = sparkDF.withColumnRenamed(column, __normalize_str(column))
  dbutils.fs.rm("dbfs:" + path_dbfs, True)  # True to delete recursively
  return sparkDF

# COMMAND ----------

final_df = await excelToParquet(adl_uld, file_name_xlsx, sheet_name)
