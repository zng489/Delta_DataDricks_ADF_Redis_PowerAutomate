...
tables = {"schema":"", "table":"", 
          "trusted_path_1":"/oni/observatorio_nacional/plataforma_produtividade/empresas_engajadas/", 
          "business_path_1":"/oni/bases_referencia/municipios/", 
          "business_path_2":"/senai/STI/producao/fechamentos_publicados/",
...
# BASE B+P
df_brasil_mais_produtivo = spark.read.parquet(adl_trusted_1)
...
### IMPORTANDO BASE B+P
# path_base_unica='{uri}/uds/unimercado/base_brasil_mais_produtivo/Consolidado_2019&2024_LM_EE_B+P.csv'.format(uri=var_adls_uri)
base_brasil_mais_produtivo = df_brasil_mais_produtivo
...
# DBTITLE 1,Validação CNAE Principal e Secundário
...
brasil_mais_produtivo_consumo2024 = base_brasil_mais_produtivo \
  .drop("ANO") \
  .withColumn("ANO", substring(col("DATA"), -4, 4)) \
  .filter((f.col("MANUFATURA_ENXUTA") == "Sim") | (f.col("EFICIENCIA_ENERGETICA") == "Sim")) \
...
base_unica_Industria_ativas_smei_vinc_sengajadas = base_unica_Industria_ativas_smei_vinc \
  .join(brasil_mais_produtivo_consumo2024, f.col('CD_CNPJ') == f.col('CNPJ_TEXTO'), "left") \
  .withColumn("Já é cliente?", f.when(f.col("CNPJ_TEXTO").isNotNull(), "Sim").otherwise("Não"))
...
base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado = base_unica_Industria_ativas_smei_vinc_sengajadas \
  .select("SG_UF", "DS_MUNICIPIO", "NM_MESORREGIAO", "DS_CAPITAL", "NM_MICRORREGIAO", "CD_CNPJ", "CD_CNPJ_BASICO", "NM_RAZAO_SOCIAL_RECEITA_EMPRESA", "DS_PORTE_EMPRESA", "PORTE_SEBRAE", "CD_CNAE20_SUBCLASSE_RFB", "CD_CNAE20_SUBCLASSE_RAIS",  "NM_TIPO_LOGRADOURO", "NM_LOGRADOURO", "NM_NUMERO_ESTBL", "NM_COMPLEMENTO", "NM_BAIRRO", "CD_CEP", "NM_DDD_1", "NM_TELEFONE_1", "NM_DDD_2", "NM_TELEFONE_2",  "CD_NATUREZA_JURIDICA_RFB","DS_NAT_JURIDICA_RFB", "CD_CNAE20_DIVISAO", "QT_VINC_ATIV", "NM_EMAIL","DS_MATRIZ_FILIAL", "Já é cliente?") \
    ...
    # "CONSUMIU SENAI STI 2023?",

# COMMAND ----------

# DBTITLE 1,>>>>> CONSTRUCAO DADOS B+P PARA CONSUMO <<<<<<
base_brasil_mais_produtivo_filtrado_enriquecido = base_brasil_mais_produtivo \
.join(base_unica, base_brasil_mais_produtivo["CNPJ_TEXTO"] == base_unica["CD_CNPJ"],"left") \
.select(
'SG_UF', 'CNPJ_TEXTO', 'CD_CNPJ_BASICO', 'NM_RAZAO_SOCIAL_RECEITA_EMPRESA', 'DS_PORTE_EMPRESA', 'PORTE_SEBRAE', 'CD_CNAE20_SUBCLASSE_RFB', 'CD_CNAE20_SUBCLASSE_RAIS', 'NM_TIPO_LOGRADOURO', 'NM_LOGRADOURO', 'NM_BAIRRO', 'CD_NATUREZA_JURIDICA_RFB','DS_NAT_JURIDICA_RFB', 'CD_CNAE20_DIVISAO', 'DATA', 'UF', 'ALI', 'MANUFATURA_ENXUTA', 'EFICIENCIA_ENERGETICA') \
.withColumn(
        "Porte_final",
        f.when(
            f.col("DS_PORTE_EMPRESA").isNotNull(),
...
# DBTITLE 1,construcao base lm e ee
...
base_brasil_mais_produtivo_filtrado_enriquecido_lm_porte_nacionalizadaliza = base_brasil_mais_produtivo_filtrado_enriquecido_lm_porte_nacionalizadaliza \
...
base_brasil_mais_produtivo_filtrado_enriquecido_lm_natjur_nacionalizada = base_brasil_mais_produtivo_filtrado_enriquecido_lm_natjur_nacionalizada \
...
base_brasil_mais_produtivo_filtrado_enriquecido_lm_cnae_nacionalizada = base_brasil_mais_produtivo_filtrado_enriquecido_lm_cnae_nacionalizada \
...
base_brasil_mais_produtivo_filtrado_enriquecido_ee_porte_nacionalizada = base_brasil_mais_produtivo_filtrado_enriquecido_ee_porte_nacionalizada \
...
base_brasil_mais_produtivo_filtrado_enriquecido_ee_natjur_nacionalizada = base_brasil_mais_produtivo_filtrado_enriquecido_ee_natjur_nacionalizada \
...
base_brasil_mais_produtivo_filtrado_enriquecido_ee_cnae_nacionalizada = base_brasil_mais_produtivo_filtrado_enriquecido_ee_cnae_nacionalizada \
...
# DBTITLE 1,>>>>>> Construção base B+P completa <<<<<<
base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado_completa = base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado \
  .join(base_brasil_mais_produtivo_filtrado_enriquecido_lm_porte_nacionalizadaliza, base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado["Porte_final"] == base_brasil_mais_produtivo_filtrado_enriquecido_lm_porte_nacionalizadaliza["Porte_LM"],"left") \
  .join(base_brasil_mais_produtivo_filtrado_enriquecido_lm_natjur_nacionalizada, base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado["CD_NATUREZA_JURIDICA_RFB"] == base_brasil_mais_produtivo_filtrado_enriquecido_lm_natjur_nacionalizada["NatJur_LM"],"left") \
  .join(base_brasil_mais_produtivo_filtrado_enriquecido_lm_cnae_nacionalizada, base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado["CD_CNAE20_DIVISAO"] == base_brasil_mais_produtivo_filtrado_enriquecido_lm_cnae_nacionalizada["CNAE_LM"],"left") \
  .join(base_brasil_mais_produtivo_filtrado_enriquecido_ee_porte_nacionalizada, base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado["Porte_final"] == base_brasil_mais_produtivo_filtrado_enriquecido_ee_porte_nacionalizada["Porte_EE"],"left") \
  .join(base_brasil_mais_produtivo_filtrado_enriquecido_ee_natjur_nacionalizada, base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado["CD_NATUREZA_JURIDICA_RFB"] == base_brasil_mais_produtivo_filtrado_enriquecido_ee_natjur_nacionalizada["NatJur_EE"],"left") \
  .join(base_brasil_mais_produtivo_filtrado_enriquecido_ee_cnae_nacionalizada, base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado["CD_CNAE20_DIVISAO"] == base_brasil_mais_produtivo_filtrado_enriquecido_ee_cnae_nacionalizada["CNAE_EE"],"left")

# COMMAND ----------

# DBTITLE 1,Lista Transformação Digital
...
# DBTITLE 1,CARGA DADOS CNAE - TRANSF DIGITAL old
# Iniciar a sessão Spark
spark = SparkSession.builder \
    .appName("cnae_transf_digital_Example") \
    .getOrCreate()

# Definir o esquema
schema_cnae_transf_digital = StructType([
    StructField("Porte", StringType(), True),
    StructField("CNAE", StringType(), True),
    StructField("Valor_CNAE_Transformacao", StringType(), True)
])
...
# Criar o DataFrame
df_cnae_calculo_td = spark.createDataFrame(data=dados_cnae_td, schema=schema_cnae_transf_digital)

# Concatenar colunas
df_cnae_calculo_td = df_cnae_calculo_td \
    .withColumn(
        "CNAE_Concat_Tranformacao",
        concat(col("Porte"), lit("_"), col("CNAE"))
    ) \
    .drop("Porte", "CNAE")

# COMMAND ----------

# DBTITLE 1,CARGA DADOS NATUREZA JURIDICA - TRANSF DIGITAL old
# Iniciar a sessão Spark
spark = SparkSession.builder \
    .appName("natjur_transf_digital_Example") \
    .getOrCreate()

# Definir o esquema
schema_natjur_transf_digital = StructType([
    StructField("Porte", StringType(), True),
    StructField("Natureza_Juridica", StringType(), True),
    StructField("Valor_NaturezaJuridica_Transformacao", StringType(), True)
])

# Dados
dados_natjur_td = [
    ("Media","Sociedade Anônima Fechada", "0,24"),
    ("Media","Sociedade Empresária Limitada", "0,76")
]

# Criar o DataFrame
df_natjur_calculo_td = spark.createDataFrame(data=dados_natjur_td, schema=schema_natjur_transf_digital)

# Concatenar colunas
df_natjur_calculo_td = df_natjur_calculo_td \
    .withColumn(
        "NatJur_Concat_Tranformacao",
        concat(col("Porte"), lit("_"), col("Natureza_Juridica"))
    ) \
    .drop("Porte","Natureza_Juridica")


# COMMAND ----------

# Verifica se os mínimos são None e atribui um valor padrão caso sejam
...
# Min_Percentual_NatJur_TranfDigital = base_unica_Industria_ativas_smei_vinc_sengajadas_filtrado_completa.agg(f.min("Percentual_NatJur_TranfDigital")).collect()[0][0] or 0.0001
...
.drop("Percentual_Porte_Lean", "Percentual_NatJur_Lean", "Percentual_CNAE_Lean", "Percentual_Porte_Eficiencia", "Percentual_NatJur_Eficiencia", "Percentual_CNAE_Eficiencia", "Classificação M3 - Lean Manufacturing Nota","Classificação M3 - Eficiência Energética Nota","Classificação M4 - Transformação Digital Nota", "Percentual_Porte_Lean_Decimal", "Percentual_NatJur_Lean_Decimal", "Percentual_CNAE_Lean_Decimal", "Percentual_Porte_Eficiencia_Decimal","Percentual_NatJur_Eficiencia_Decimal", "Percentual_CNAE_Eficiencia_Decimal")
