...
tables = {"schema":"", "table":"", "raw_path":"/usr/oni/mte/novo_caged/identificada/","trusted_path":"/oni/mte/novo_caged/identificada/", "prm_path": "/tmp/dev/prm/usr/oni/mte/novo_caged_identificada/FIEC_me_novo_caged_exc_for_mov_mapeamento_unificado_trusted.xlsx"}
...
df_caged = df.select(*__transform_columns())
...
df_caged = df_caged.withColumn('CD_ANO_COMPETENCIA', f.substring(f.col('CD_ANO_COMPETENCIA'), 0, 4).cast(IntegerType()))


# COMMAND ----------

df_caged = df_caged.withColumn('CD_SUBCLASSE', f.when(f.length(f.col('CD_CNAE20_SUBCLASSE'))<7, f.lpad('CD_CNAE20_SUBCLASSE', 7, "0")).otherwise(f.col('CD_CNAE20_SUBCLASSE')))
