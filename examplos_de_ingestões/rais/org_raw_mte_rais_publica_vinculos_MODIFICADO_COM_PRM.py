...
# Databricks notebook source
# Databricks notebook source
var_param_folder = {'namespace':'oni','file_folder':'mte/rais/publica/vinculos/2023','prm_path':'/tmp/dev/prm/usr/oni/mte/rais_publica_vinculos/MTE_RAIS_PUBLICA_VINCULO_mapeamento_raw.xlsx', 'extension':'TXT', 'column_delimiter': ';', 'encoding': 'UTF-8', 'null_value': ''}
...
var_dls = { "folders": { "landing": "/tmp/dev/uld", "error": "/tmp/dev/err", "staging": "/tmp/dev/stg", "log": "/tmp/dev/log", "raw": "/tmp/dev/raw", "archive": "/tmp/dev/ach" }, "systems": { "raw": "usr" }, "path_prefix": "" }
...
for column in set(df.columns).difference(list_of_prm_columns):
  print(f'{datetime.now()} >>> Variável {column} não mapeada no PRM. Adicionando ao dicionário como string.')
  if var_prm_year not in var_prm_dict:
    var_prm_dict[var_prm_year] = []
  var_prm_dict[var_prm_year].append(f"['{column}','{column}','string']")
...
headers = {'name_header':'Campo Origem','pos_header':'C','pos_org':'C','pos_dst':'E','pos_type':'F'}
var_prm_dict = cf.parse_ba_doc_spark(dbutils, var_file_prm, headers=headers, sheet_names='2023')
...
column_mapping = var_prm_dict['2023']

# Define the function to rename columns
def rename_columns(df, column_mapping):
    column_dict = {original: new for original, new, _ in column_mapping}
    renamed_df = df.select([df[col].alias(column_dict.get(col, col)) for col in df.columns])
    return renamed_df
