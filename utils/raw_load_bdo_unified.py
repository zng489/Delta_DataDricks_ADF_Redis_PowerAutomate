...
# MAGIC 
# MAGIC "env" variable is not needed here. Pipeline in ADF will decide upon it and downstream the correct values.
# MAGIC 
# MAGIC Needed variables from ADF are (and all of them are dict):
# MAGIC - **table**: array\(dict\)
# MAGIC - **adf**: dict
# MAGIC - **dls**: dict
...
"""
var_options = {
    "adls_uri": var_adls_uri,
    "control_fields": {
        "nm_arq_in": functions.lit(None).cast("integer"),
        ...
        "kv_process_control": functions.lit(json.dumps(var_adf)).cast("string"),
    },
}
...
# DBTITLE 1,Adding partitioning to table definition
