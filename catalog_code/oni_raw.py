...
#           'abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net/trs/indicadores/', 'abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net/trs/fred/', 'abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net/trs/trello/',
#           'abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net/trs/power_bi/','abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net/trs/jira_service_management/',
#           'abfss:datalake@cnibigdatadlsgen2.dfs.core.windows.net/trs/devops/','abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net/trs/datalake/','abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net/trs/azure_ad/',#'abfss:datalake@cnibigdatadlsgen2.dfs.core.windows.net/raw/usr/sti/',
#           'abfss:datalake@cnibigdatadlsgen2.dfs.core.windows.net/raw/usr/power_bi/',
#           'abfss:datalake@cnibigdatadlsgen2.dfs.core.windows.net/raw/usr/jira_service_management/',
#           'abfss:datalake@cnibigdatadlsgen2.dfs.core.windows.net/raw/usr/devops/',
#           'abfss:datalake@cnibigdatadlsgen2.dfs.core.windows.net/trs/raw/usr/datalake/',
#           'abfss:datalake@cnibigdatadlsgen2.dfs.core.windows.net/raw/usr/azure_ad/']

#from pyspark.sql import Row
#rdd1 = sc.parallelize(out_from_list)
#row_rdd = rdd1.map(lambda x: Row(x))
#df=sqlContext.createDataFrame(row_rdd,['OUT_OF_'])
#df.display()
...
df_paths = df2
df_paths.createOrReplaceTempView('raw_paths')
...
for i in df4:
  ...
  try:
    ...
    Q = Q.count()
    #count.append({'counts':Q})
  except:
    print(i)
  count.append({'counts':Q})

df_ct = spark.createDataFrame(data=count)
df_ct.select(sum('counts')).show()
...
SELECT raw_paths.AUTORIZAÇÃO, raw_size.size_bytes, raw_paths.PATHS, raw_paths.PATHS, raw_size.path
FROM raw_paths 
LEFT JOIN raw_size
on raw_paths.PATHS = raw_size.path

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC -- select * from raw_size where size_bytes = null
# MAGIC -- select * from raw_size
# MAGIC -- select * from raw_paths
# MAGIC
# MAGIC -- with CAMADA_RAW as ()
# MAGIC --(SELECT *, raw_paths.PATHS, raw_size.path
# MAGIC --FROM raw_paths 
# MAGIC --LEFT JOIN raw_size
# MAGIC --                on raw_paths.PATHS = raw_size.path)
# MAGIC
# MAGIC SELECT AUTORIZACAO,PATHS, size_bytes FROM (
# MAGIC   SELECT raw_paths.AUTORIZACAO, raw_size.size_bytes, raw_paths.PATHS, raw_paths.PATHS, raw_size.path
# MAGIC FROM raw_paths 
# MAGIC LEFT JOIN raw_size
# MAGIC on raw_paths.PATHS = raw_size.path)
