...
t = ['/raw/crw/aco_brasil/prod_aco_nacional/',
...
for F in t:
  ...
  data_size.append({'path':path,'size_bytes':str(total_size)}) 
data_size   
...
for x in t:
  ...
  path = f'{var_adls_uri}/{i}'
  print(path)
  for a in dbutils.fs.ls(path):
    for 

# COMMAND ----------

dbutils.fs.ls('abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net/biz/uniepro/fta_saeb_alunos_kpi/ID_PROVA_BRASIL=2011')


# COMMAND ----------

    for c in dbutils.fs.ls(a):
      print(c.size)
...
dbutils.fs.ls('abfss://datalake@cnibigdatadlsgen2.dfs.core.windows.net/raw/crw/aco_brasil/prod_aco_nacional/')
