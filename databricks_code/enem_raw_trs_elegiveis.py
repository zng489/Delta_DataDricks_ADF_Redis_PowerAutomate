...
path = '{uri}/raw/crw/inep_enem/microdados_enem/NU_ANO=2019/'.format(uri=var_adls_uri)
...
df_test = pd.DataFrame(df.select('*').collect())
...
# file trs_biz_fta_enem_agregado_notas
...
adf = {'adf_factory_name': 'development', 
...
# MAGIC %md 
# MAGIC <h1 style="color:red";> org_raw_microdados_enem </h1>
...
df_agg = (df
      .groupby('NU_ANO', 'CO_ENTIDADE','NU_INSCRICAO')
      .agg(f.count(f.lit(1)).alias('VL_TOT_ALUNOS_ENEM_ESCOLA'),
           f.round(f.avg((f.col('NU_NOTA_CH') + f.col('NU_NOTA_CN') + f.col('NU_NOTA_LC') + f.col('NU_NOTA_MT')) / f.lit(4)), 5).alias('NU_NOTA_GERAL'),
           f.round(f.avg((f.col('NU_NOTA_CH') + f.col('NU_NOTA_CN') + f.col('NU_NOTA_LC') + f.col('NU_NOTA_MT') + f.col('NU_NOTA_REDACAO')) / f.lit(5)), 5).alias('NU_NOTA_GERAL_REDAC'),
           f.round(f.avg(f.col('NU_NOTA_CH')), 5).alias('VL_NOTA_CH'), 
...
df_agg_pandas = df_agg.filter(df_agg.CO_ENTIDADE == 14000520).collect()

display(df_agg_pandas)
#df_agg_pandas.toPandas()
